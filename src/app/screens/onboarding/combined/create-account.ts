...
import { LfCreateAccount } from '../law-firm/create-account';
import { StCreateAccount } from '../student/create-account';

@Component({
  selector: 'app-compare-create-account',
  standalone: true,
  imports: [RouterLink, LfCreateAccount, StCreateAccount],
  template: `
    <div class="sticky top-0 z-50 flex items-center gap-6 border-b border-border bg-background/95 px-6 py-3 backdrop-blur-sm">
      ...
      <span class="text-sm font-semibold text-foreground">Create Account</span>
    ...
    <div style="transform: translateZ(0)">
      @if (tab() === 'lf') {
        <app-lf-create-account />
      } @else {
        <app-st-create-account />
      }
    </div>
  `,
})
export class CompareCreateAccount {
